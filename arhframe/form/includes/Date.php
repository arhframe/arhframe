...
class Zebra_Form_Date extends Zebra_Form_Control
{
     ...
     *  @return void
     */
    public function Zebra_Form_Date($id, $default = '', $attributes = '')
    {
     ...
     *  @return void
     */
    public function disabled_dates($disabled_dates)
    {
        // set the date picker's attribute
     ...
     *  @return void
     */
    public function disable_zebra_datepicker()
    {
     ...
     *  @return void
     */
    public function first_day_of_week($day)
    {
     ...
     *  @return void
     */
    public function readonly_element($value)
    {
        // set the date picker's attribute
     ...
     *  @return void
     */
    public function show_week_number($value)
    {
        // set the date picker's attribute
     ...
     *  @return void
     */
    public function weekend_days($days)
    {
        // set the date picker's attribute
     ...
     *  @return void
     */
    public function zero_pad($state)
    {
        // set the date picker's attribute
        ...
        if (!isset($this->limits)) {
            ...
            if (isset($this->first_selectable_date) && $this->_is_disabled($first_selectable_year, $first_selectable_month, $first_selectable_day)) {
                ...
                while ($this->_is_disabled($first_selectable_year, $first_selectable_month)) {
                    ...
                    } elseif ($first_selectable_month < 1) {
     ...
     *  @access private
     */
    public function _is_format_valid($date)
    {
        ...
        if (!empty($regexp)) {
            ...
            if (preg_match($format, preg_replace('/\s/', '', $date), $segments)) {
     ...
     *  @access private
     */
    public function _is_disabled($year, $month = '', $day = '')
    {
        ...
        if (isset($this->disabled_dates)) {
            ...
            foreach ($this->disabled_dates as $rule) {
                ...
                if (in_array($year, $rule[2]) || in_array('*', $rule[2]))

                    // if the rules apply for the current month
                    if (($month != '' && in_array($month, $rule[1])) || in_array('*', $rule[1]))

                        // if the rules apply for the current day
                        if (($day != '' && in_array($day, $rule[0])) || in_array('*', $rule[0])) {


                            // if day is to be disabled whatever the day
                            // don't look any further
                            if (in_array('*', $rule[3])) return ($disabled = true);
