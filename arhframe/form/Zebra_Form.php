...
define('ZEBRA_FORM_UPLOAD_RANDOM_NAMES', "random");
 ...
 * @version    2.8.9e (last revision: January 17, 2013)
...
class Zebra_Form
{
     ...
     * @var    string
     */
    public $file_upload_permissions;
     ...
     * @return void
     */
    public function Zebra_Form($name, $method = 'POST', $action = '', $attributes = '')
    {
     ...
     * @return void
     */
    public function add_error($error_block, $error_message)
    {
     ...
     * @return void
     */
    public function assets_path($server_path, $url)
    {
     ...
     *  elements with random content *only* if the element does not already has a default value!</samp>
     *
     ...
     * @return void
     */
    public function auto_fill($defaults = array(), $specifics_only = false)
    {
     ...
     * @return void
     */
    public function captcha_storage($method)
    {
     ...
     */
    public function client_side_validation($properties)
    {
     ...
     * @return void
     */
    public function clientside_validation($properties)
    {
     ...
     * @return void
     */
    public function csrf($csrf_storage_method = 'auto', $csrf_token_lifetime = 0, $csrf_cookie_config = array('path' => '/', 'domain' => '', 'secure' => false, 'httponly' => false))
    {
     ...
     * @return void
     */
    public function language($language, $isDefault = false)
    {

        // include the language file
        $file = rtrim(dirname(__FILE__), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'languages/' . trim($language) . '.php';
        if (!is_file($file) && !$isDefault) {
            return $this->language(Config::getInstance()->config->defaultLang, true);
        } elseif (!is_file($file)) {
            return $this->language('en_EN');
        }
        require rtrim(dirname(__FILE__), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'languages/' . trim($language) . '.php';
    ...
    public function render($template = '', $return = false, $variables = '')
    {
        ...
        if ($template == '' || $template == '*horizontal' || $template == '*vertical') {
            ...
            foreach ($this->controls as $control) {
                ...
                //
                //  the control if the control is not a label that is to be displayed inside the control as it's
                // default value
                if (!isset($attributes['inside']))
        ...
        $controls["Form_Footer"] =
            '</form><script type="text/javascript">function ' . $function_name . '() {if(typeof jQuery=="undefined"||typeof jQuery.fn.Zebra_Form=="undefined"' .
        ...
        $output .= $controls["Form_Footer"];
        $finalArray = array_merge($this->variables, $this->errors, $controls);
        if ($template == "twig") {
            return $finalArray;
        } else {
            return array('header' => $controls["Form_Header"], 'content' => $output);
        }
     ...
     * @return                 void
     */
    public function show_all_error_messages($value = false)
    {
     ...
     * @return boolean             Returns TRUE if every rule was obeyed, FALSE if not.
     */
    public function validate_control($control)
    {
     ...
     * @access private
     */
    public function _csrf_generate_token($force = false)
    {
     ...
     * @access private
     */
    public function _csrf_validate()
    {
     ...
     * @access private
     */
    public function _convert($control, $type, $jpeg_quality = 85, $preserve_original_file = false, $overwrite = false)
    {
     ...
     * @access private
     */
    public function _extract_values($array)
    {
     ...
     * @access private
     */
    public function _load_mime_types()
    {
     ...
     * @access private
     */
    public function _resize($control, $prefix, $width, $height, $preserve_aspect_ratio = true, $method = ZEBRA_IMAGE_BOXED, $background_color = 'FFFFFF', $enlarge_smaller_images = true, $jpeg_quality = 85)
    {
     ...
     * @access private
     */
    public function _validate_dependencies($id, $referer = array())
    {
     ...
     * @access private
     */
    public function _upload($control, $upload_path, $filename = true)
    {
        ...
        //inal path where to upload the file, relative to the DOCUMENT_ROOT
        $path = $upload_path;
