...
class File
{
    ...
    {
        $folder = $this->folder;
        if (!is_dir($this->folder) && $folder[0] != '/' && DIRECTORY_SEPARATOR == '/') {
            $folder = '/' . $folder;
        }
        return $folder;
    }
    ...
    public function getContent()
    {
        ...
        if (!$this->isFile()) {
            throw new ArhframeException("File '" . $this->absolute() . "' doesn't exist.");
        }
        ...
        if (!feof($handle)) {
            throw new ArhframeException("Error: unexpected fgets() fail\n");
        }
        fclose($handle);
        return $return;
    }
